...
// Safe text component
const SafeText = ({ children, className = "" }: { children: string | number | null | undefined; className?: string }) => {
                ...
                <div className="flex items-start space-x-3 sm:space-x-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight text-slate-900 break-words">
                      <SafeText>{patientDetails.firstName}</SafeText> <SafeText>{patientDetails.lastName}</SafeText>
                    </h1>
                    <div className="mt-2 space-y-1 sm:space-y-2">
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-slate-600">
                        <div className="flex items-center space-x-1">
                          <Mail className="w-4 h-4 flex-shrink-0" />
                          <SafeText className="break-all">{patientDetails.email}</SafeText>
                        </div>
                        <div className="flex items-center space-x-1">
                          <Phone className="w-4 h-4 flex-shrink-0" />
                          <SafeText>{patientDetails.phone}</SafeText>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mt-4">
                    {[
                      ...
                      <div key={idx} className="bg-white rounded-lg p-4 border border-slate-200">
                        ...
                        <SafeText className="text-base text-slate-900 font-medium break-all">{value}</SafeText>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="bg-slate-50 rounded-xl p-4 sm:p-6">
                    ...
                    <div className="mt-4">
                      <div className="bg-white rounded-lg p-4 border border-slate-200 inline-block">
                        ...
                        <SafeText className="text-base text-slate-900 font-medium">{patientDetails.occupation}</SafeText>
                      </div>
                    </div>
                  </div>
                )}
                                ...
                                <div className="flex items-center space-x-3 w-full">
                                  ...
                                  <div className="flex-1 text-left min-w-0">
                                    <div className="font-medium text-slate-800 group-hover:text-emerald-800 truncate">
                                      <SafeText>{doc.doctorName}</SafeText>
                                    </div>
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center space-x-3 p-3 bg-slate-50 rounded-lg">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="font-medium text-slate-800">
                                  <SafeText>{item.appointmentFormattedDate}</SafeText>
                                </div>
                                <div className="flex items-center space-x-2 mt-1">
                                  <Clock className="w-3 h-3 text-slate-500" />
                                  <span className="text-sm text-slate-600">
                                    <SafeText>{item.appointmentTime}</SafeText>
                                  </span>
                                </div>
                              </div>
                            </div>
                            ...
                            {item.doctorName && (
                              <div className="flex items-center space-x-2 text-sm text-slate-600">
                                ...
                                <span>Dr. <SafeText>{item.doctorName}</SafeText></span>
                              </div>
                            )}
